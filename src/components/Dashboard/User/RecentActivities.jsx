        ...
        {activities.map((activity, index) => {
          ...
          return (
            <div key={index} className="flex items-center justify-between py-2 border-b">
              <span style={{ color: theme.palette.text.secondary }}>{activity.title}</span>
              <span className="text-sm" style={{ color: theme.palette.text.secondary }}>{formattedDate}</span> {/* Display formatted date and time */}
            </div>
          );
        })}
      </div>
